...
2. Choose "From a manifest" and use this:
   ...
   features:
     ...
     slash_commands:
       ...
       - command: /jules-repo
         url: https://jules-slack-mention.YOUR_SUBDOMAIN.workers.dev/slack/events
         description: Set your preferred GitHub repository
         should_escape: false
   oauth_config:
     scopes:
       bot:
...
4. Go to **Slash Commands** and create `/jules-token` and `/jules-repo` with the same request URL
...
2. **Set your preferred repository (optional)**:

   ```bash
   /jules-repo google/jules
   ```

   By default, Jules will use the first available repository. Use this command to switch or `/jules-repo clear` to reset.

3. **Create a task by @mentioning**:

   ```bash
   @Jules Fix the login bug in the authentication module
   ```

4. **Or DM the bot directly** with your task
